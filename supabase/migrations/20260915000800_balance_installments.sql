...
begin
  ...
  select r.total_amount-coalesce(sum(a.amount),0)
    into outstanding
  ...
  outstanding:=coalesce(outstanding,r.total_amount);
  ...
  for installment in 1..p_installments loop
    ...
    values(
      ...
      due_on+((installment-1)*interval '1 month')
    );
  end loop;
...
grant execute on function public.configure_reservation_balance(uuid,integer,date) to authenticated;
