...
CREATE TABLE IF NOT EXISTS public.reservation_passengers (
    ...
    passenger_id UUID NULL, -- Vínculo opcional ao cadastro
    
    -- Snapshots (Imutabilidade histórica)
...
-- 5. Holds de Capacidade (Locks Temporários)
CREATE TABLE IF NOT EXISTS public.seat_holds (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    agency_id UUID NOT NULL,
    trip_id UUID NOT NULL,
    
    -- Se for hold de assento específico, seat_id é preenchido. 
    -- Se for hold de vaga geral (sem mapa), seat_id é NULL.
    seat_id UUID NULL, 
...
-- 6. Atribuição de Assentos Definitiva
...
-- 7. Pedidos de Aprovação (Exceções)
...
-- Policies (Simplificadas para agência)
