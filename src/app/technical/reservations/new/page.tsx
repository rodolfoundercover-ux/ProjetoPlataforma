...
export default function NewReservationPage() {
  ...
  useEffect(() => {
    async function loadInitialData() {
      // Aqui buscaríamos de APIs reais. Simulação de carregamento:
      const resTrips = await fetch("/api/technical/trips"); // Exemplo
      // setTrips(await resTrips.json());
      
      // Para fins de demo, usaremos dados simulados se as APIs não estiverem prontas
      setTrips([
      ...
      setCustomers([
        { id: "cust-1", full_name: "João Silva" },
  ...
  const handleAddPassenger = async () => {
    ...
    try {
      const resPass = await reservationService.addPassengerToReservation({
        ...
        boarding_point_name: "Rodoviária Centro",
        list_price: 100,
        sale_price: 100,
        discount_amount: 0,
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-8">
      ...
      {step === 1 && (
        <div className="p-6 border rounded-xl bg-card shadow-sm space-y-4">
          <h2 className="text-lg font-semibold">Passo 1: Configurações Básicas</h2>
          ...
          >
            {loading ? "Processando..." : "Avançar para Passageiros"}
          </button>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="p-6 border rounded-xl bg-card shadow-sm space-y-6">
          <h2 className="text-lg font-semibold">Passo 3: Revisão Final</h2>
