...
export default async function TeamPage() {
  ...
  return <><h1>Equipe e permissões</h1><p>Os dados exibidos e as alterações são autorizados pelo banco para cada agência.</p>
    {!memberships?.length ? <p role="status">Você não possui vínculo ativo com uma agência.</p> : <ul>{memberships.map(m => <li key={m.agency_id}>{m.agencies?.name ?? "Agência"} — {m.role}</li>)}</ul>}
    <h2>Criar perfil de permissões</h2><form action={createPermissionProfile}><label>Agência<select name="agencyId" required>{memberships?.map(m => <option key={m.agency_id} value={m.agency_id}>{m.agencies?.name ?? m.agency_id}</option>)}</select></label><label>Nome do perfil<input name="name" maxLength={80} required /></label><button type="submit">Criar perfil</button></form>
    ...
    <h2>Convites</h2>{!invitations?.length?<p>Nenhum convite visível.</p>:<ul>{invitations.map(i=><li key={i.id}>{i.email} — {i.status} — vence em {new Date(i.expires_at).toLocaleString("pt-BR")} {i.status==="PENDING"&&<form action={revokeInvitation}><input type="hidden" name="invitationId" value={i.id}/><button>Revogar</button></form>}</li>)}</ul>}
